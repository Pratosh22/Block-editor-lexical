...
//         (e) => e.aliases.includes(name)
//       )?.emoji;
//       if (emoji) {
//         textNode.replace(
//           $createTextNode(emoji)
//         );
//       }
//     },
//     trigger: ":",
...
export const TABLE: ElementTransformer =
  {
    ...
    replace: (
      ...
      // Header row
      if (
        TABLE_ROW_DIVIDER_REG_EXP.test(
          match[0]
        )
      ) {
        const table =
...
function getTableColumnsSize(
  table: TableNode
) {
  const row = table.getFirstChild();
  return $isTableRowNode(row)
    ? row.getChildrenSize()
    : 0;
}
